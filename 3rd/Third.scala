...
def doStuff = {
    val parsed1 = parseList(bigInput1)
    val listpos1 = getListOfPositions(parsed1, 0, (0,0), List((0,0)) )
    val parsed2 = parseList(bigInput2)
    val listpos2 = getListOfPositions(parsed2, 0, (0,0), List((0,0)) )

    val common = listpos1.filter(x => listpos2.contains(x) && x != (0,0)).map(el => (el, listpos1.indexOf(el) + listpos2.indexOf(el)))
    val leastDist = common.map(x => x._1.abs + x._2.abs).min
    val leastSteps = common.minBy(_._2)
}
